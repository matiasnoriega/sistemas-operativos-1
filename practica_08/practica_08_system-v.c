...
int main(int argc, char *argv[]){
	...
	printf("Creamos el semáforo y el espacio de memoria compartida...\n\n");	
	/* Obtenemos el semáforo para sincronizar la escritura de la memoria compartida */
	if((semid = semget(IPC_PRIVATE, 1, 0600)) < 0){
		perror("semget");
		exit(EXIT_FAILURE);
	}

	/* Inicializamos el semaforo en 1*/
